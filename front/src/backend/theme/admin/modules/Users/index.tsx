...
import { fetchUsersApi, deleteUsersApi } from '../../../../state/data-layer/users';
import { fetchUsers, usersCount } from '../../../../state/selectors';
...
const User = (props: any) => {
    ...
    useEffect(() => {
        if (searchQuery) {
            props.fetchUsersApi({ limit: rowsPerPage ? rowsPerPage : 20, page: page > -1 ? page+1 : 1, search: searchQuery});
            ...
            <Box sx={{ width: '100%', padding: '10px', margin: "0px" }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                    <Typography variant="h6">
                        <p style={{ margin: '0', fontWeight: 800 }}>User</p>
                    ...
                    onClick={()=>{
                        if(selected && selected.length > 0 && users.length > 0) {
                            const data = users.filter((user)=>selected.indexOf(user.id) > -1);
                            try {
                                const opts = {};
                                const parser = new Parser(opts);
                                const csv = parser.parse(data);
                                const csvData = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                                FileSaver.saveAs(csvData, `users[${data.length}].csv`);
                ...
                <Paper sx={{ width: '100%', borderTop:'1px solid #dddddd',  borderLeft:'1px solid #dddddd',  borderRight:'1px solid #dddddd',  }}>
                    ...
                    <TableContainer sx={{ width: ' 100%', margin: 0, padding: 0, borderRadius: "5px" }}>
                            ...
                            <TableBody>
                                {stableSort(users, getComparator(order, orderBy))
                                    .map((row: any, index: number) => {
                                        ...
                                        const role = row.role && row.role.id ? row.role.name : ''
                                        return (
                                            <TableRow
                        ...
                        >
                            <MenuItem onClick={(e) => {
                                navigate(`/user/edit/${currentSelected}`);
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={props.usersCount ? props.usersCount : 0}
                ...
                <ConfirmationDialog
                    ...
                    onConfirm={() => {
                        if (currentSelected) {
                            props.deleteUsersApi({ id: currentSelected });
                        }
                        setDelConfirmDialog(false);
                    }}
                ></ConfirmationDialog>
...
const mapDispatchState = (dispatch: any) => {
    return {
        fetchUsersApi: (payload: any) => dispatch(fetchUsersApi(payload)),
        deleteUsersApi: (payload: any) => dispatch(deleteUsersApi(payload)),

    };
};
export default connect(mapStateToProps, mapDispatchState)(User)
