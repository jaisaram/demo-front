...
import { deleteBankMasterApi, fetchBankMasterApi, uploadBankCsvApi } from "../../../../state/data-layer/bankMaster";
...
import { bankMastersCount, fetchBankMasters, fetchUsers } from "../../../../state/selectors";
...
const Bank = (props: any) => {
    ...
    const [bankMasters, setBankMasters] = React.useState<Data[]>([]);
    ...
    useEffect(() => {
        if (searchQuery) {
            props.fetchBankMasterApi({ limit: rowsPerPage ? rowsPerPage : 20, page: page > -1 ? page + 1 : 1, search: searchQuery });
            setBankMasters(props.bankMasters);
        } else {
            props.fetchBankMasterApi({ limit: rowsPerPage ? rowsPerPage : 20, page: page > -1 ? page + 1 : 1, search: searchQuery });
            setBankMasters(props.bankMasters);
        }
    ...
    useEffect(() => {
        if (csvArray && csvArray.length > 0) {
            props.uploadBankCsvApi({ banks: csvArray });
            setDragfile(null);
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = bankMasters.map((n) => n.id);
    ...
    const handleMenuItemClick = (
        ...
        if (index === 1) {
            if (selected && selected.length > 0 && bankMasters.length > 0) {
                const data = bankMasters.filter((master) => selected.indexOf(master.id) > -1);
                try {
                    const opts = {};
                    const parser = new Parser(opts);
                    const csv = parser.parse(data);
                    const csvData = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                    FileSaver.saveAs(csvData, `bank-masters[${data.length}].csv`);
            ...
            <Box sx={{ width: '100%', padding: '10px', margin: "0px" }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                    <Typography variant="h6" sx={{ flexGrow: 1 }}>
                        <p style={{ margin: '0', fontWeight: 800 }}>Bank Master</p>
                        <p style={{ margin: '0', fontSize: "14px" }}>Bank Master data list, add, update and delete actions performs here: </p>
                    </Typography>
                    ...
                    <Model isOpen={isOpenImport}
                        ...
                        title={"Import CSV"} description={"Import banks details using csv file with Name, Category, Status fields."}
                    ...
                    >
                        ...
                        <br></br>
                        <Link download={"bank-masters-template.csv"} href="/assets/images/bank-masters-template.csv">Download Temaplate CSV</Link>
                    </Model>
                ...
                <Paper sx={{ width: '100%', borderTop: '1px solid #dddddd', borderLeft: '1px solid #dddddd', borderRight: '1px solid #dddddd', }}>
                    ...
                    <TableContainer sx={{ width: ' 100%', margin: 0, padding: 0, borderRadius: "5px" }}>
                        <Table
                            ...
                            <EnhancedTableHead
                                ...
                                rowCount={bankMasters.length}
                            />
                            <TableBody>
                                {stableSort(bankMasters, getComparator(order, orderBy))
                                    .map((row, index) => {
                        ...
                        >
                            <MenuItem onClick={(e) => {
                                navigate(`/bank/edit/${currentSelected}`);
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={props.bankMastersCount ? props.bankMastersCount : 0}
                ...
                <ConfirmationDialog
                    ...
                    onConfirm={() => {
                        if (currentSelected) {
                            props.deleteBankMasterApi({ ids: [currentSelected] });
                        } else if (selected && selected.length > 0) {
                            props.deleteBankMasterApi({ ids: selected });
                        }
                        setDelConfirmDialog(false);
                    }}
                ></ConfirmationDialog>
...
const mapStateToProps = (state: any) => ({
    ...
    bankMasters: fetchBankMasters(state),
    bankMastersCount: bankMastersCount(state),
})

const mapDispatchState = (dispatch: any) => {
    return {
        fetchBankMasterApi: (payload: any) => dispatch(fetchBankMasterApi(payload)),
        deleteBankMasterApi: (payload: any) => dispatch(deleteBankMasterApi(payload)),
        uploadBankCsvApi: (payload: any) => dispatch(uploadBankCsvApi(payload)),
        errorNotice: (payload: any) => dispatch(errorNotice(payload)),

    };
};
export default connect(mapStateToProps, mapDispatchState)(Bank)
