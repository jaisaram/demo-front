...
import { deleteRoleApi, fetchRolesApi } from "../../../../state/data-layer/roles";
...
import { roleCountSelector, fetchRolesSelector, getUser, fetchUsers } from "../../../../state/selectors";
...
const Role = (props: any) => {
    ...
    useEffect(() => {
        if (searchQuery) {
            props.fetchRoleApi({ limit: rowsPerPage ? rowsPerPage : 20, page: page > -1 ? page+1 : 1, search: searchQuery});
            ...
            <Box sx={{ width: '100%', padding: '10px', margin: "0px" }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                    <Typography variant="h6" sx={{ flexGrow: 1 }}>
                        <p style={{ margin: '0', fontWeight: 800 }}>Role</p>
                        <p style={{ margin: '0', fontSize: "14px" }}>Role data list, add, update and delete actions performs here: </p>
                    </Typography>
                    <Button variant="contained" style={{ textTransform: 'none', marginRight: '10px' }} onClick={()=>{
                        if (props.currentUser && props.currentUser.role && props.currentUser.role.name !== 'Administrator') {
                ...
                <Paper sx={{ width: '100%', borderTop:'1px solid #dddddd',  borderLeft:'1px solid #dddddd',  borderRight:'1px solid #dddddd',  }}>
                    ...
                    <TableContainer sx={{ width: ' 100%', margin: 0, padding: 0, borderRadius: "5px" }}>
                        ...
                        >
                            <MenuItem onClick={(e) => {
                                navigate(`/user/role/edit/${currentSelected}`);
                ...
                <ConfirmationDialog
                    ...
                    onConfirm={() => {
                        if (currentSelected) {
                            props.deleteRoleApi({ id: currentSelected });
                        }
                        setDelConfirmDialog(false);
                    }}
                ></ConfirmationDialog>
...
const mapDispatchState = (dispatch: any) => {
    return {
        ...
        deleteRoleApi: (payload: any) => dispatch(deleteRoleApi(payload)),
        errorNotice:  (payload: any) => dispatch(errorNotice(payload)),

    };
};
export default connect(mapStateToProps, mapDispatchState)(Role)
